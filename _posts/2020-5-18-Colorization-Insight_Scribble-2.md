...
where \\(F_A\\) is normally a constant; \\(F_G=-\epsilon \kappa\\) such that \\(\kappa\\) is the local curvature of the evolving curve \\(T\\) and \\(\epsilon\\) is a constant; \\(h\\) is a filter or halting component, to terminate the curve evolution.
