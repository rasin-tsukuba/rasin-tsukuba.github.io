...
- 一个**图**\\(G(V, E)\\) 由顶点集\\(V={V_1, \dots, V_n}\\)与边集 \\(E={e_{ij}}^n_{i,j=1}\\) 构成，图的邻接矩阵S则由每条边的权值 \\(S_{ij}\geq0\\)。如果图是无向图，则 \\(S_{ij}=S_{ji} \forall i, j \in [n]\\)。 边的权值 \\(S_{ij}\\)表示 \\(v_i\\)和\\(v_j\\)的相似度，由特定的评价函数得出，值越高则两个点越相似。
