...
例如，在医学数据分析问题中，由于数据样本的不足和不平衡，每个医学机构中的数据集可能不足以生成高质量的预测结果。然而，由于隐私问题，很难将原始医学数据样本与其他机构的数据集中在一起。是否可以通过适当的映射将原始数据转换到另一（例如低纬）空间。由于中间表示的每个特征都没有现实解释，因此映射的数据（中间表示）可以相当容易地集中化。
...
另外，令 \\({X}_i = [{x}_{i1}, {x}_{i2}, \cdots, {x}_{in_i}] \in \mathbb{R}^{m\times n_i}\\) 为第\\(i\\)个训练数据集。对监督学习来说，我们额外令 \\({L}_i = [{l}_{i1}, {l}_{i2}, \cdots, {l}_{in_i}] \in \mathbb{R}^{l \times n_i}\\) 为训练数据的正确标签。另外，\\(s_i\\)为第\\(i\\)个数据集的测试数据及数量，即:

$$
s = \sum_{i=1}^d s_i
$$

第\\(i\\)个数据集的测试数据集可以表示为 \\({Y}_i = [{y}_{i1}, {y}_{i2}, \cdots, {y}_{is_i}] \in \mathbb{R}^{m\times s_i}\\)。
